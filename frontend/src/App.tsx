...
import { ChartIcon, HistoryIcon, TemplateIcon } from "./components/icons";
import { authClient } from "./auth-client";

type Phase = "idle" | "uploading" | "processing" | "done" | "error";
type View = "dashboard" | "verify" | NavKey;
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {view === "dashboard" && (
        <Dashboard onVerify={goVerify} onOpenHistory={() => setView("history")} />
      )}
            ...
            <div className="space-y-6">
              ...
              {report.llm?.error && report.llm.enabled && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-xs text-amber-700">
                  LLM layer note: {report.llm.error}. Deterministic analyzers still produced the
                  full report.
                </div>
              )}
                        ...
                        f.id === activeId ? (
                          <div key={f.id}>
                            ...
                            <p className="mt-1 text-sm capitalize text-slate-600">
                              Severity: <span className="font-semibold">{f.severity}</span> ·
                              module: {f.module}
                            </p>
